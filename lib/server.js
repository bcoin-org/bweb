...
class Server extends EventEmitter {
  ...
  init() {
    ...
    this.http = http.createServer(options);
  ...
  route() {
    this.http.on('request', async (hreq, hres) => {
      ...
      try {
        ...
        await this.handleError(e, req, res);
      }
    });
  }
  ...
  async handleMounts(req, res) {
    ...
    for (const hook of this.mounts) {
      ...
      try {
        ...
        try {
          await server.handleError(e, req, res);
        } finally {
          req.navigate(url);
        }
        return true;
